...
This overrides `chalk:voice`'s top-of-artefact rule, and only here: a terminal scrolls upward, so in a chat reply the last thing written is the first thing read. Everything else it says about a tl;dr still holds.
