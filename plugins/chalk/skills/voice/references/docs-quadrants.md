...
- **A closing "you should now see …" check**.
...
- **Rationale MUST stay out of the body.**
  This is the one quadrant where chalk's usual "always capture the why" does not apply inline. The *why* — tradeoffs, motivation, upgrade story — belongs in an adjacent explanation section or changelog block, not next to the definition.
...
- **"You should see X" after each step**.
...
A bad tutorial *strands the reader* and they don't come back.
