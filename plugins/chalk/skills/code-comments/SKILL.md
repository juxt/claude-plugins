...
- **A non-public surface takes the implementation rules whatever its markup.**
  A kdoc on a private function still has to earn its place.
...
- **Simplify the code.**
  **If what you've written is the simplest way of satisfying the constraints, it doesn't need a comment.**
  If it isn't the simplest way, make it simpler and the comment goes with it.
...
A yes is necessary, not sufficient: the comment still has to pass the test below.
...
- **A constant has no derivation.**
  A timeout, a retry count, a buffer size, a threshold — where the number came from, not what it is.
